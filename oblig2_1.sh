...
do
	case $i in
		...
		3) echo "Det finnes $(ps axms --no-headers | wc -l) prosesser og tråder.";;
		4) echo "Det var $(vmstat 1 2 | tail -1 | awk '{print $12}') context switcher siste sekund.";;
		5) echo "Usermode: $(vmstat 1 2 | tail -1| awk '{print $13}') og kernel mode: $(vmstat 1 2 | tail -1| awk '{print $13}')";;
